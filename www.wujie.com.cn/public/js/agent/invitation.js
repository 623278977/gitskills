...
Zepto(function() {
	...
	var args = getQueryStringArgs(),
		...
		orter_uid = args['receiver_agent_id'], //被分享投资人的经纪人id
	...
	function getdetail(id, uid) {
		...
		param['agent_id'] = uid;
		param['customer_agent_id'] = orter_uid;
		...
		ajaxRequest(param, url, function(data) {
			if(data.status) {
				...
				$('.chat').attr('real_name',k.realname);
				if(data.message) {
					...
					conHtml += '<div class="btn"><button class="btn-l border_xm" real_name= "'+k.realname+'"><img src="/images/agent/sms_03.png" class="tel_img"/></button>';
					if(k.has_tel == 0){
						conHtml += '<button class="btn-r border_xm tel_none"><img src="/images/agent/tel.png" class="tel_img"/></button></div>';
					}else {
...
$(document).on('click','.address_list',function(){
	...
	addressBook(id,orter_uid,levelId);
	refreshContacts();
});
...
function addressBook(id,orter_uid,level){
	var params = {};
	params['agent_id'] = orter_uid;
	params['customer_id'] = id;
	params['customer_level'] = level;
	var url = labUser.agent_path + '/customer/add-share-customer-info/_v010001';
	ajaxRequest(params,url,function(data){
		if(data.status){
			tips(data.message);
			setTimeout(function(){
