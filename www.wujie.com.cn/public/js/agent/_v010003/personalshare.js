...
Zepto(function() {
	...
	function getdetail(uid) {
		...
		ajaxRequest(param, url, function(data) {
			if(data.status) {
				if(data.message) {
					$('#container').attr('logo',data.message.avatar);
					//个人信息
					var myHtml = '';
...
//						$('.ul_share_span').text(data.message.realname);
					}else {
						myHtml += '<p class="dis_con mb08"><span class="b f15 mb1">' + data.message.nickname + '</span></p>';
//						$('.ul_share_span').text(data.message.nickname);
					};
...
////			 	var name = $(this).children('.ul_share_span');
////			 	$('.share-li-sel').attr('name',name);
			...
			//分享页
				...
				if(isiOS) {
					...
					$(document).on('click','.xm-btn-invest', function() {
						window.location.href = labUser.path + '/webapp/agent/register/registercustomer/_v010100?agent_id=' + uid;
					});
					//成为经纪人
					$(document).on('click','.xm-btn-agent', function() {
						window.location.href = labUser.path + '/webapp/agent/letter/registeragent/_v010100?agent_id=' + uid;
					});
					oppenIos();
				} else if(isAndroid) {
					//成为投资人
...
//成为经纪人
//$(document).on('click', '.xm-btn-agent', function() {
//	window.location.href = labUser.path + '/webapp/agent/letter/registeragent/_v010003?agent_id=' + uid;
//});
//成为投资人
//$(document).on('click ', '.xm-btn-invest', function() {
//	window.location.href = labUser.path + '/webapp/agent/register/registercustomer/_v010003?agent_id=' + uid;
//});
	//打开本地--Android
	...
	function oppenIos() {
 	...
 	$(document).on('click','.xm-acting',function(){
		...
		//分享页跳转
		window.location.href = labUser.path + '/webapp/brand/detail/_v020900?uid=0&id='+brand_id+'&is_share=1';
//		if(shareFlag){
