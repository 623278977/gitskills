...
Zepto(function (){
              ...
              if(obj.length>0){
                $.each(obj,function(k,v){
                  var html='';
                      ...
                      html+='<div class="f13 ui_contrack_top fline ui_pR">等待'+' '+(v.realname?v.realname:v.nickname)+' '+'确定'+' '+'['+(v.brand.length>6?v.brand.substr(0,6)+'…':v.brand)+']'+' '+'付款协议<span class="ffa300 fr">等待中</span></div>';
                    ...
                    if(v.pre_pay){
                      ...
                      html+='<div class="ui_pR color666">\
                                ...
                                <span><span class="ffa300">待确认</span><span class=" color333 padding">'+'(&nbsp'+v.leftover +'&nbsp)'+'</span></span><span class="fr ui_upordown" data-id="'+v.id+'">关闭</span> \
  ...
  //出现缺省页方法
   function experience(){
      var bigo=$('.ui_con>.ui_top_time').length==0?true:false;
      if(bigo){
        $('#nocommenttip3').removeClass('none');
      }
    }
    $(document).on('click','.ui_upordown',function(){
             $(this).parent().parent().parent().addClass('none').prev().remove();
             var id=$(this).data('id');
             shutUpcontract(id);
             experience();
