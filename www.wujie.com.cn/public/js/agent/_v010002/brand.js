...
Zepto(function() {
    ...
    var brandDetail = {
        ...
        detail: function(id, uid) {
            ...
            var url = labUser.agent_path + '/brand/detail/_v010002';
    ...
    function getlessonDetail(result){
        ...
        if(videos.length > 0){
            $.each(videos,function(i,j){
                vidHtml += '<li class="A_videos_intro l" data-id="'+j.id+'"><div class="A_videos_img mb05">';
                if(j.is_read == 0){
                    vidHtml += '<img src="'+j.image+'" style="height:100%;"></div>';
                }else{
                    vidHtml += '<img src="'+j.image+'" style="height:100%;"><img src="/images/agent/learned.png" class="learned_logo"></div>';
                };
                vidHtml += '<div class="" ><p class="f13 mb05 color333 nowrap">'+j.title+'</p>';
                vidHtml += '<p class="f11 color666 nowrap mb1">'+j.summary+'</p></div></li>';     
            }) ;
            vidHtml += '<div class="clearfix"></div>'
        ...
        if(news.length > 0){
            $.each(news,function(index ,item){
                newHtml += '<li class="tl fline mb1-33" data-id="'+item.id+'"><p class="f14 b">'+item.title+'</p><p class="f12 color999">'+item.summary+'</p>';
                if(item.is_read == 0){
                    newHtml += '<div class="lh35 color999 mt1"><span class="l border-top999 f12">未打卡</span>';
                }else{
                     newHtml += '<div class="lh35 color999 mt1"><span class="l border-top999 cfd4d4d f12">已打卡</span>';
                };
                newHtml += '<span class="r f11">所属权：'+result.brand.title+'</span></div><div class="clearfix"></div></li>';
            });
