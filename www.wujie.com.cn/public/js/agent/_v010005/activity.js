...
//分享到微信
 function showShare(){
    var  type = 'activity',
         ...
         header = '活动',
         content = '我在无界商圈发现一个不错的项目，想邀请你一起参加！',
         begintime = $('#act_des').data('begintime'),
         citys = $('#citys').text(),
         actid=$('#act_name').data('act_id'),
         sharecontent=$('#act_name').data('sharecontent');
    if(sharecontent){
         shareOut(title, url, img, header, (sharecontent.length<20?sharecontent:sharecontent.substr(0,20)+'…'), begintime, citys,actid,type);  
    }else{
...
Zepto(function (){
    ...
    var activityDetail = {
        detail: function (activity_id) {
            ...
            activityDetail.attend(data.message);
            }
            });
        },
        ...
        attend:function(obj){
              if(obj.is_sign==1){
                 $('.ui_share li').eq(0).css('background','#ccc').text('已报名');
              }else{
                 signActivity();
              }
        },
        share:function(){
            if(shareFlag){
                ...
                $('.ui_share').eq(0).hide();
                $('.fixedbottom').hide();
                $('.ui_share').eq(1).find('li').eq(1).on('click',function(){
    ...
    function commonHTML(selfObj, is_share) {
        ...
        if(title.length>20){
         ...
         $('#act_name').html(title).data('sharecontent',selfObj.share_summary).data('content',selfObj.detail);   
        ...
        if(newtime>selfObj.end_time){
            ...
            $('.fixedbottom').text('活动已结束').css('background','#ccc').removeClass('none');
            $('.ui_share').hide();
        }
        $('.ui_share li').eq(1).data('title',selfObj.title).data('imgurl',selfObj.share_image);
    ...
    //经纪人自己报名agentAttendactivity()命名
   function agentAttendactivity(activity_id){
        if (isAndroid) {
            javascript: myObject.agentAttendactivity(activity_id);
         }
        else if (isiOS) {
            var data = {
                'activity_id': activity_id
    ...
    function signActivity(){
         $('.ui_share li').eq(0).on('click',function(){
