//ByHongky
    ...
    var messageDetail = {
        init: function (id) {
            var param = {};
                param["id"]=id;
                param["uid"]=agent_id;
            var url=labUser.agent_path+'/article/detail/_v010005';
        ...
        data:function(obj){
             ...
             commonReply(obj.detail.news_id);
        ...
        getcommentList:function(id,uid,page,page_size){
                        ...
                        var url=labUser.agent_path+'/article/comment-list/_v010005';
                        ajaxRequest(param, url, function (data) {
                        if (data.status){
                          ...
                          if(data.message.data!=''){
                            $.each(data.message.data,function(k,v){
                                ...
                                if(v.c_uid == uid){
                                    ...
                                    html += '<span class="comment_tip none"><em class="reply" data-id="'+v.id+'" data-type="mes">回复</em><em class="copy">复制</em></span></p>';
                                }
                                if(v.pId){
    ...
    //文章点赞
     ...
     $(document).on('click','.ui-link-title p',function(){
        var id=$(this).data('id');
        if(shareFlag){
         window.location.href=labUser.path+'webapp/agent/hotmessage/detail/_v010005?id='+id+'&agent_id='+agent_id+'&is_share=1';
        }else{
          window.location.href=labUser.path+'webapp/agent/hotmessage/detail/_v010005?id='+id+'&agent_id='+agent_id;   
        }
     })
     //点击加载更多
