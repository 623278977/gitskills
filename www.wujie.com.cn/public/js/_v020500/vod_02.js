		...
		ajaxRequest(params,url,function(data){
			if(data.status){
				...
				if(shareFlag){
					...
					if(sel.price==0){
						...
						// 试看一分钟
						getVod(sel.video_url, 60,0,sel.price,long,shareFlag);
					}else{
                    ...
                    //点击相关视频跳转页面
         			$(document).on('tap',".more_video>li",function(){
         				...
         				window.location.href=labUser.path+'webapp/vod/detail/_v020502?id='+id+'&uid=0&is_share=1&share_mark'+origin_mark;
         			});	
					...
					// var ovoid=sel.maker_ids.split('@')[0];
         			...
         			$(document).on('tap ',".more_video>li",function(){
         				var id=$(this).attr('data_id');
         				window.location.href=labUser.path+'webapp/vod/detail/_v020502?id='+id+'&uid='+param.uid+'&pagetag=05-4';
         			});
         			// 点击视频预览试看一分钟
					$(document).on('tap ','#know_more',function(){
						$('.share_video').hide();
						$("#video_box").show();
						getVod(sel.video_url, 60,param.uid,sel.price,long,shareFlag);
					})
         			//点击电话
			...
			// 点击活动跳转至活动详情页
				$(document).on('tap','.act',function(){
					if(shareFlag){
						window.location.href=labUser.path+'webapp/activity/detail/_v020600?id='+sel.activity_id+'&uid='+params.uid+'&is_share=1&share_mark='+origin_mark;
					}else{
						window.location.href=labUser.path+'webapp/activity/detail/_v020600?id='+sel.activity_id+'&uid='+params.uid+'&pagetag=02-2';
					}
			...
			// 点击品牌跳转至品牌详情页
				$(document).on('tap','.brand-company',function(){
					if(shareFlag){
						window.location.href=labUser.path+'webapp/brand/detail/_v020600?id='+sel.brand_id+'&uid='+params.uid+'&is_share=1&share_mark='+origin_mark;
					}else{
						window.location.href=labUser.path+'webapp/brand/detail/_v020600?pagetag=08-9&id='+sel.brand_id+'&uid='+params.uid;
					}
