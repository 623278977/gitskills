    ...
    function createNavBar(objLive, type, shareFlag) {
        ...
        if (type == 'living') {
            if (objLive.with_brand == '1') {
                ...
                navHtml += '   <span class="threecol green" id="detail_block">直播概况</span>\
                               <i class="left33">|</i>\
                ...
                navHtml += '<span class="twocol green" id="detail_block">直播概况</span>\
                               <i class="left50">|</i>\
        ...
        if (type == 'preview') {
            ...
            if (!shareFlag) {
                ...
                if (objLive.share_reward_unit == 'score') {//积分
                    $('#share_video').append('<div class="pl1-33 pr1-33 fx_share"><p class="f12 l">分享直播，立即获得' + objLive.share_reward_num + '积分</p><button class="c00a0ff l f12 understand"><img src="http://mt.wujie.com.cn//images/notice.png" alt="">了解更多分享机制</button><span class="f16 r close_share">×</span></div>');
                }
                else if (objLive.share_reward_unit == 'currency') {
                    $('#share_video').append('<div class="pl1-33 pr1-33 fx_share"><p class="f12 l">分享直播，立即获得' + objLive.share_reward_num + '无界币</p><button class="c00a0ff l f12 understand"><img src="http://mt.wujie.com.cn//images/notice.png" alt="">了解更多分享机制</button><span class="f16 r close_share">×</span></div>');
                }
                //事件
                fx();
            }
        ...
        var rHtml = '<div class="remindpart" id="registerpart" style="z-index:197;">\
                <div class="content">\
                    ...
                    <div class="userinput remindcolor">\
                        ...
                        <div class="putdiv remindcolor height06"><input type="text" name="phonenumber" placeholder="手机号" id="zcphone"/></div>\
    ...
    function liveEndEvent(shareFlag) {
        if (shareFlag) {
            ...
            $(document).on('click', '.livevideo', function () {
                ...
                window.location.href = labUser.path + "webapp/vod/detail/_v020500?id=" + livevideo_id + "&uid=" + uid + "&pagetag=05-4&is_share=1";
            });
            //资讯
            ...
            $(document).on('click', '.livevideo', function () {
                var livevideo_id = $(this).data('livevideo_id');
                window.location.href = labUser.path + "webapp/vod/detail/_v020500?id=" + livevideo_id + "&uid=" + uid + "&pagetag=05-4";
            });
            //资讯
            $(document).on('click', '.livemessage', function () {
                var message_id = $(this).data('message_id');
    ...
    function creatLoadApp(onebtn) {
        ...
        if (onebtn) {
            loadApp = '<div class="fixed_btn" id="loadAppBtn">\
                          <button class="signup" id="loadapp" style="width:100%;float: right;background-color: #1e8cd4;color:#fff;">下载APP</button>\
                       </div>';
        }
        else {
            loadApp = '<div class="fixed_btn" id="loadAppBtn">\
                          <button class="reserve"  id="reserve" style="width:50%;background-color: white;color:#6bc24b;">设置直播提醒</button>\
                          <button class="signup" id="loadapp" style="width:50%;float: right;background-color: #1e8cd4;color:#fff;">下载APP</button>\
                       </div>';
        }
                    ...
                    <div class="content">\
                        ...
                        <div class="userinput remindcolor">\
                            <div class="f12 putdiv">\
                                <div class="pdiv">\
                                    <p>直播名称：' + objLive.subject + '</p>\
                                    <p>直播开始时间：' + unix_to_fulltime(objLive.begin_time) + '</p>\
                                </div>\
                            </div>\
