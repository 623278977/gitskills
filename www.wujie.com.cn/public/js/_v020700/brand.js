...
Zepto(function() {
    ...
    $(document).on('click','.toAct',function(){
        ...
        window.location.href = labUser.path +'webapp/activity/detail/_v020700?id='+actID+'&pagetag=02-2&uid='+uid + shareUrl;
    })
        // 按钮点击查看公司详情
    $(document).on('click', '#company_details', function() {
            window.location.href = labUser.path + 'webapp/brand/company/_v020700?id=' + id + '&uid=' + uid + shareUrl;
        })
        //创业基金的关闭按钮
    $(document).on('click tap', '#packet_close', function() {
        ...
        //APP的发送加盟意向点击弹出
    $(document).on('click', '#brand_suggest,#join', function() {
        fadeBrand('#brand-mes', 'a-fadeinT');
        $('.brand-message').css('z-index', '199');
        var obj={
            'type':'brand',
            'id':id
        };
...
//提交意向加盟时获取用户详情
    ...
    function getDetail(result) {
        ...
        // var fav = result.relation.is_favorite;
        // var length = result.videos.length;
        if (!shareFlag) {
            if (brand.fund == 0) {
                $('.brand-np').removeClass('none');
                $('#creat_fund').addClass('none');
                if(brand.distribution_id == 0 || result.is_distribution == 0){
                    $('.brand-np #get_coin').remove();
                    $('.brand-np #brand_suggest').css('width','66.6666%');
                }
            } else {
                $('.brand-p').removeClass('none');
                if(brand.distribution_id == 0 || result.is_distribution == 0){
                    $('.brand-p #get_coin').remove();
                    $('.brand-p>div').css('width','33.3333%');
                }
            }
        }

        //是否显示分销赚佣
       if(brand.distribution_id > 0 && (!shareFlag) && result.is_distribution == 1){
            var ruleHtml='';
            $.each(result.distribution,function(i,j){
                ruleHtml += j.describe +';' ;
            });
        ...
        // $('#wrapper').data('fav',fav);
        // $('#wrapper').data('isVideo',length);
   ...
   $(document).on('click','#get_coin,.getcoin',function(){
    ...
    $(document).on('click','#relativevideo li',function(){
        ...
        if(shareFlag){
            window.location.href = labUser.path +'webapp/vod/detail/_v020700?id='+id+'&uid='+uid+'&share_mark='+origin_mark+'is_share=1';
        }else{
            window.location.href = labUser.path +'webapp/vod/detail/_v020700?id='+id+'&uid='+uid+'&pagetag=05-4';
        }
...
function showShare() {
    ...
    var pageUrl =labUser.path+ 'webapp/brand/detail/_v020700?id=' + id + '&uid=' + uid;
    ...
    if($('#brand_name').data('distribution') > 0){
        var args = getQueryStringArgs(),
                id = args['id'] || '0';
        var pageUrl =labUser.path+ 'webapp/brand/detail/_v020700?id=' + id + '&uid=' + uid+ '&share_mark=' + $('#brand_name').data('mark');//用来追踪原始分享者
