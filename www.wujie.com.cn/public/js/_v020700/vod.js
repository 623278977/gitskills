...
;var Video = {
   // 加载相关信息
	vodDetail:function(param,shareFlag){
		...
		var url=labUser.api_path + '/video/detail/_v020700';
		ajaxRequest(params,url,function(data){
			if(data.status){
				...
				if(sel.distribution_id==0 || data.message.is_distribution == 0){
   		             $('#share').data('reward',0);
	            }else{
	                $('#share').data('reward',1);  
	            ...
	            //是否展示分享赚佣模块
	                var ruleHtml='';
			            $.each(data.message.distribution,function(i,j){
			                ruleHtml += j.describe +';' ;
			            });
			            // console.log(ruleHtml.match(/[0-9]{1,6}积分|[0-9]{1,6}元|\\s{0,}\/\\s{0,}(人|单)?/g));
			            ruleHtml = ruleHtml.replace(/[0-9]{1,6}积分|[0-9]{1,6}元(人|单)?/g,'<em class="colorf4">$&</em>');
            ...
            //                                 }
                    ...
                    //点击相关视频跳转页面
         			$(document).on('tap',".more_video>li",function(){
         				...
         				window.location.href=labUser.path+'webapp/vod/detail/_v020700?id='+id+'&uid=0&is_share=1&share_mark'+origin_mark;
         			});	
...
// 点击品牌跳转至品牌详情页
		$(document).on('tap','.brand-company',function(){
			var brandID =$('#brand_info').attr('data_brandID');
			if(shareFlag){
				window.location.href=labUser.path+'webapp/brand/detail/_v020700?id='+brandID+'&uid='+uid+'&is_share=1&share_mark='+origin_mark;
			}else{
				window.location.href=labUser.path+'webapp/brand/detail/_v020700?pagetag=08-9&id='+brandID+'&uid='+uid;
			}
		})
//点击提交意向
