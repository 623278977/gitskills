        ...
        if (is_share) {
            ...
            if (timestamp < selfObj.end_time) {
                ...
                $(document).on('click', '.signup,.wjbrk', function () {
                    ...
                    var site_ticket_count=$('#act_name').data('site_ticket_count');
    ...
    function commonHTML(selfObj, is_share) {
        ...
        if (selfObj.brand.length > 0) {
            ...
            $.each(selfObj.brand, function (index, item) {
                ...
                brandHtml += ' <div class="white-bg brand-company pl1-33 fline position2" data-brand_id="' + item.id + '">';
                ...
                brandHtml +='<div id="back" class="back none"><div id="shoucang" class="cang"></div><span id="shoucangson">收藏</span>';
                brandHtml+='<div id="tijiao" class="tijiao"></div><span id="tijiaoson">提交意向</span>';
                brandHtml+='<div id="chakan" class="chakan"></div><span id="chakanson">查看详情</span>';
                brandHtml+='</div>';
                brandHtml+='</div>';//我加的
            });
