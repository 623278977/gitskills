        ...
        detail: function(activity_id, uid) {
            ...
            ajaxRequest(param, url, function(data) {
                if (data.status) {
                    ...
                    var distribution_id = data.message.distribution_id;
                    var rule = data.message.distribution;
                    var is_distribution = data.message.is_distribution;
                    if (distribution_id > 0 && (!shareFlag)) {
                        var ruleHtml = '';
                    ...
                    var dis = $('.dis_coin');
                    var spanHeight = parseFloat(dis.css('height')),
                        spanLH = parseFloat(dis.css('line-height'));
                    if (spanHeight > spanLH * 2) {
                        dis.addClass('ui-nowrap-multi');
                        // dis.removeClass('pb1-5');
                    } else {
                        //$('.more_icon').addClass('none');
                        //dis.addClass('pb1-5');
                    };
                    var dis_coin = $('.dis_coin'),
                        more_icon = $('.more_icon img'),
                        showMore = true;
