...
Zepto(function() {
    ...
    $(document).on('click', '#company_details', function() {
            window.location.href = labUser.path + 'webapp/brand/company?id=' + id + '&uid=' + uid + shareUrl;
        })
        //创业基金的关闭按钮
    ...
    $(document).on('click', '#brand_toquestion', function() {
        ...
        window.location.href = labUser.path + 'webapp/brand/questions?id=' + id + '&uid=' + uid +'&pagetag=025-1'+shareUrl;
        onEvent('brand_detail_question')
    });
    //图文详情里的推荐品牌点击跳转
    $(document).on('click', '.brand-companys', function() {
        if(shareFlag){
            var b_id = $(this).data('id');
            window.location.href = labUser.path + 'webapp/brand/detail/_v020500?id=' + b_id + '&uid=0&pagetag=08-9'+ shareUrl;;
        }
        else{
            var b_id = $(this).data('id');
            if (isiOS) {
                ...
                window.location.href = labUser.path + 'webapp/brand/detail/_v020500?id=' + b_id + '&uid=' + uid+'&pagetag=08-9'+ shareUrl;;
            }
        }
    });
        //意向留言
    ...
    $(document).on('click', '.share-send-mes', function() {
        ...
        share_mark = $('#brand_name').attr('data-mark');
        ...
        message: function(id, uid, mobile, realname, consult, type, share_mark, content_type) {
            ...
            ajaxRequest(param, url, function(data) {
                if (data.status) {
                    ...
                    $('.fixed-bg').click();
                    $('.b-reset').click();
                     onEvent('brand_detail_intent_submit');
                    if (isiOS) {
                        $('.fixed-bg').addClass('none');
    ...
    function getMore(result) {
        ...
        // console.log(result.brands.length);
        $.each(result.brands, function(i, item) {
               ...
               if (i>=5) {
                ...
                str += '</p>';
                for (var k = 0, j = item.keywords.length; k < j; k++) {
                    str += '<a class="tags-key border-8a-radius">' + item.keywords[k] + '</a>';
                }
                str += '</div>';
...
function showShare() {
    ...
    var pageUrl =labUser.path+ 'webapp/brand/detail/_v020500?id=' + id + '&uid=' + uid;
    var img = $('#brand_logo').attr('src');
    var header = '';
    var content = "项目："+$('#brand_name').text()+'\r\n'+"行业："+$('#category_name').text();
    ...
    if($('#brand_name').data('distribution') > 0){
        var args = getQueryStringArgs(),
                id = args['id'] || '0';
        var pageUrl =labUser.path+ 'webapp/brand/detail/_v020500?id=' + id + '&uid=' + uid+ '&share_mark=' + $('#brand_name').data('mark');//用来追踪原始分享者
