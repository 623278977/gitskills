            ...
            if (data.status) {
                ...
                if (objLive.end_time < timenow) {
                    if (shareFlag) {
                        ...
                        contentHtml += '</div>';
                        contentHtml += '</section>';
                        $('#containerBox').html(contentHtml).css('visibility','visible');
                        //to publisher
                        ...
                        $(document).on('click', '.brandcontent', function () {
                            ...
                            window.location.href = labUser.path + "webapp/brand/detail/_v020400?id=" + brand_id + "&uid=" + uid + "&pagetag=02-1-2";
                        });
                        //to activity-detail
                        $(document).on('click', '#actdetail', function () {
                            var id = $(this).data('activity_id');
                            window.location.href = labUser.path + "webapp/activity/detail/_v020400?id=" + id + "&uid=" + uid + "&pagetag=02-2&position_id=0&makerid=0";
                        });
                    }
                    increaseViewn(id, 'live');
                }
                else {
                    $('#video_box').css('height', parseInt(clientWidth * 9 / 16));
                    //detail-info
                    ...
                    if (shareFlag) {
                        ...
                        $(document).on('click', '.act-block', function () {
                            var id = $(this).data('activity_id');
                            window.location.href = labUser.path + "webapp/activity/detail/_v020400?id=" + id + "&uid=" + uid + "&makerid=0&position_id=0&is_share=1";
                        });
                        //to recommend live
                        $(document).on('click', ".recommend-video", function () {
                            var id = $(this).data('liveid');
                            window.location.href = labUser.path + "webapp/live/detail/_v020400?id=" + id + "&uid=" + labUser.uid + "&pagetag=04-9&is_share=1";
                        });
                        //close fast register
                        ...
                        var openHeight = Math.floor($('#installapp').height());
                        var videoHeight = parseInt(clientWidth * 9 / 16);
                        $('#video_box').css('height', videoHeight);
                        $('#livecount').css('top', Math.floor(videoHeight + openHeight)-2);
                        var tabtop = openHeight + videoHeight + $('#livecount').height();
                        $('#videonews_box').css('padding-top', tabtop);
                        $('#share_video').css('height', tabtop);
                        $('#daojishi').css('height', tabtop);
                        $('#wrapper').css('top', tabtop + parseInt($('#columns').height()));
                        $(".containerBox").css('visibility', 'visible');
                    }
                    else {
                        $(".containerBox").css('visibility', 'visible');
                        $('#video_box').css('height', parseInt(clientWidth * 9 / 16));
                        $('#livecount').css('top', parseInt(clientWidth * 9 / 16));
                        var tabtop = parseInt(clientWidth * 9 / 16) + $('#livecount').height();
                        $('#videonews_box').css('padding-top', tabtop);
                        $('#share_video').css('height', tabtop);
                        $('#daojishi').css('height', tabtop);
                        $('#wrapper').css('top', tabtop + parseInt($('#columns').height()));
                        ...
                        $(document).on('click', '.act-block', function () {
                            var id = $(this).data('activity_id');
                            window.location.href = labUser.path + "webapp/activity/detail/_v020400?id=" + id + "&uid=" + uid + "&pagetag=02-2&makerid=0&position_id=0&version=2.3";
                        });
                        //to recommend live
                        $(".recommend-video").tap(function () {
                            var id = $(this).data('liveid');
                            window.location.href = labUser.path + "webapp/live/detail/_v020400?id=" + id + "&uid=" + uid + "&pagetag=04-9&version=2.3";
                        });
                        //buy goods
                        $(document).on('click', '.canbuy', function () {
                        ...
                        if ('vip_id' in objLive) {
                            ...
                            $(document).on('tap', '#zbrow', function () {
                                var vip_id = $('#zbname').data('zbid');
                                window.location.href = labUser.path + 'webapp/special/detail/_v020400?pagetag=02-1&vip_id=' + objLive.vip_id + '&uid=' + uid;
                            });
                            //不是专版会员且没有购买
                            if (objLive.is_authorize == 0 && objLive.is_purchase == 0) {
                                ...
                                timeOutShow(objLive.begin_time, objLive.endtime, function () {
                                    ...
                                    //}
                                    $("#video_box").show();
                                    var v_height = $('#video_box').height();
                                    //切换到互动
                                    $(".column>span[type=comment]").click();
                                    showPlayer(live_url, v_height, objLive.subject, id);
                                });
                            }
                        }
                        else {//非专版
...
function liveHtml(objLive, objActivity, shareFlag) {
    ...
    var cityArray = objActivity.city.split('@');
    if (cityArray.length > 5) {
        cityArray = cityArray.slice(0, 5);
        $('#act_positon').html('活动现场：' + cityArray.join('、') + '等');
    }
    else {
