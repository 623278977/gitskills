...
class Message
{
     ...
     *
     * @param $fromUserId           发送用户ID （必传）
     * @param $toUserId             接受用户ID （必传）
     * @param $objectName           消息类型   （必传）
     * @param $content              消息内容（可以自定义消息模板） （必传）
     * @param string $pushContent   自定义显示的push内容
     * @param string $pushData
     * @param string $count
     * @param $verifyBlacklist      是否过滤发送人黑名单列表，0 表示为不过滤、 1 表示为过滤，默认为 0 不过滤
     * @param $isPersisted
     * @param $isCounted
     * @param $isIncludeSender      发送用户自己是否接收消息，0 表示为不接收，1 表示为接收，默认为 0 不接收，只有在 toUserId 为一个用户 Id 的时候有效
     * @return  $json
     * @internal param $fromUserId  :   发送人用户 Id。（必传）
     * @internal param $toUserId    :   接收用户 Id，可以实现向多人发送消息，每次上限为 1000 人。（必传）
     * @internal param $voiceMessage :  消息。
     * @internal param $pushContent  :  定义显示的 Push 内容，如果 objectName 为融云内置消息类型时，则发送后用户一定会收到 Push 信息。如果为自定义消息，则 pushContent 为自定义消息显示的 Push 内容，如果不传则用户不会收到 Push 通知。（可选）
     * @internal param $pushData     :  针对 iOS 平台为 Push 通知时附加到 payload 中，Android 客户端收到推送消息时对应字段名为 pushData。（可选）
     * @internal param $count :针对 iOS 平台，Push 时用来控制未读消息显示数，只有在 toUserId 为一个用户 Id 的时候有效。（可选）
     * @internal param $verifyBlacklist :是否过滤发送人黑名单列表，0 表示为不过滤、 1 表示为过滤，默认为 0 不过滤。（可选）
     * @internal param $isPersisted :当前版本有新的自定义消息，而老版本没有该自定义消息时，老版本客户端收到消息后是否进行存储，0 表示为不存储、 1 表示为存储，默认为 1 存储消息。（可选）
     * @internal param $isCounted :当前版本有新的自定义消息，而老版本没有该自定义消息时，老版本客户端收到消息后是否进行未读消息计数，0 表示为不计数、 1 表示为计数，默认为 1 计数，未读消息数增加 1。（可选）
     * @internal param $isIncludeSender :发送用户自已是否接收消息，0 表示为不接收，1 表示为接收，默认为 0 不接收。（可选）
     *
     */
	public function publishPrivate($fromUserId, $toUserId,  $objectName, $content, $pushContent = '', $pushData = '', $count = '', $verifyBlacklist, $isPersisted, $isCounted, $isIncludeSender) {
    	try{
			if (empty($fromUserId))
                return array('message' => '发送人用户ID为必填项', 'status' => false);

			if (empty($toUserId))
                return array('message' => '接受人用户ID为必填项', 'status' => false);
				
			if (empty($objectName))
                return array('message' => '消息类型为必填项', 'status' => false);

			if (empty($content))
                return array('message' => '消息内容为必填项', 'status' => false);
...
//                'isIncludeSender'  => 1
    		...
    		if(empty($ret))
                return array('message' => ['message' => '请求失败'], 'status' => false);
     ...
     * @return  $json
     * @internal param $fromUserId :发送人用户 Id。（必传）
     * @internal param $toUserId :接收用户 Id，提供多个本参数可以实现向多人发送消息，上限为 1000 人。（必传）
     * @internal param $txtMessage :发送消息内容（必传）
     * @internal param $pushContent :如果为自定义消息，定义显示的 Push 内容，内容中定义标识通过 values 中设置的标识位内容进行替换.如消息类型为自定义不需要 Push 通知，则对应数组传空值即可。（可选）
     * @internal param $pushData :针对 iOS 平台为 Push 通知时附加到 payload 中，Android 客户端收到推送消息时对应字段名为 pushData。如不需要 Push 功能对应数组传空值即可。（可选）
     * @internal param $isPersisted :当前版本有新的自定义消息，而老版本没有该自定义消息时，老版本客户端收到消息后是否进行存储，0 表示为不存储、 1 表示为存储，默认为 1 存储消息。（可选）
     * @internal param $isCounted :当前版本有新的自定义消息，而老版本没有该自定义消息时，老版本客户端收到消息后是否进行未读消息计数，0 表示为不计数、 1 表示为计数，默认为 1 计数，未读消息数增加 1。（可选）
     *
     */
     ...
     * @return  $json
     * @internal param $fromUserId :发送人用户 Id 。（必传）
     * @internal param $toGroupId :接收群Id，提供多个本参数可以实现向多群发送消息，最多不超过 3 个群组。（必传）
     * @internal param $txtMessage :发送消息内容（必传）
     * @internal param $pushContent :定义显示的 Push 内容，如果 objectName 为融云内置消息类型时，则发送后用户一定会收到 Push 信息. 如果为自定义消息，则 pushContent 为自定义消息显示的 Push 内容，如果不传则用户不会收到 Push 通知。（可选）
     * @internal param $pushData :针对 iOS 平台为 Push 通知时附加到 payload 中，Android 客户端收到推送消息时对应字段名为 pushData。（可选）
     * @internal param $isPersisted :当前版本有新的自定义消息，而老版本没有该自定义消息时，老版本客户端收到消息后是否进行存储，0 表示为不存储、 1 表示为存储，默认为 1 存储消息。（可选）
     ...
     * @return  $json
     * @internal param $fromUserId :发送人用户 Id。（必传）
     * @internal param $toDiscussionId :接收讨论组 Id。（必传）
     * @internal param $txtMessage :发送消息内容（必传）
     * @internal param $pushContent :定义显示的 Push 内容，如果 objectName 为融云内置消息类型时，则发送后用户一定会收到 Push 信息. 如果为自定义消息，则 pushContent 为自定义消息显示的 Push 内容，如果不传则用户不会收到 Push 通知。（可选）
     * @internal param $pushData :针对 iOS 平台为 Push 通知时附加到 payload 中，Android 客户端收到推送消息时对应字段名为 pushData.（可选）
     * @internal param $isPersisted :当前版本有新的自定义消息，而老版本没有该自定义消息时，老版本客户端收到消息后是否进行存储，0 表示为不存储、 1 表示为存储，默认为 1 存储消息.（可选）
