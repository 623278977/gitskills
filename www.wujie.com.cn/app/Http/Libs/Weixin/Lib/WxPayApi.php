...
use App\Http\Libs\Weixin\Lib\WxPayResults;
...
//            $inputObj->SetNotify_url(WxPayConfig::NOTIFY_URL);//异步通知url
            $inputObj->SetNotify_url(config('weixin.weixin.NOTIFY_URL'));//异步通知url
        }
//        $inputObj->SetAppid(WxPayConfig::APPID);//公众账号ID
        $inputObj->SetAppid(config('weixin.weixin.APPID'));//公众账号ID
//        $inputObj->SetMch_id(WxPayConfig::MCHID);//商户号
        $inputObj->SetMch_id(config('weixin.weixin.MCHID'));//商户号
        $inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);//终端ip
        //$inputObj->SetSpbill_create_ip("1.1.1.1");
//        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
        $inputObj->SetNonce_str($inputObj->GetNonce_str());//随机字符串
        //签名
        $inputObj->SetSign();
        $xml = $inputObj->ToXml();
...
//        if (WxPayConfig::REPORT_LEVENL == 0) {
        if (config('weixin.weixin.REPORT_LEVENL') == 0) {
            return;
        }
        //如果仅失败上报
//        if (WxPayConfig::REPORT_LEVENL == 1 &&
        if (config('weixin.weixin.REPORT_LEVENL') == 1 &&
...
//        if (WxPayConfig::CURL_PROXY_HOST != "0.0.0.0"
        if (config('weixin.weixin.CURL_PROXY_HOST') != "0.0.0.0"
//            && WxPayConfig::CURL_PROXY_PORT != 0
            && config('weixin.weixin.CURL_PROXY_PORT') != 0
        ) {
//            curl_setopt($ch, CURLOPT_PROXY, WxPayConfig::CURL_PROXY_HOST);
            curl_setopt($ch, CURLOPT_PROXY, config('weixin.weixin.CURL_PROXY_HOST'));
//            curl_setopt($ch, CURLOPT_PROXYPORT, WxPayConfig::CURL_PROXY_PORT);
            curl_setopt($ch, CURLOPT_PROXYPORT, config('weixin.weixin.CURL_PROXY_PORT'));
        }
