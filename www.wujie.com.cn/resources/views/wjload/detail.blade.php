...
<script> 
  Zepto(function(){
    ...
    var cookies=readCookie ('xiazai');
       if(is_weixin()){
            $(document).on("tap",".ios,.android",function(){
            ...
            $(document).on("tap",".ios,.android",function(){
              if(cookies){
                ...
                if(local){
                    ...
                    _czc.push(﻿["_trackEvent",'无界商圈','APP','ios',1]);  
                }
                writeCookie ('xiazai', '1', 24);
              }  
              setTimeout(function(){window.location.href = 'https://itunes.apple.com/app/id981501194';},800) 
            });
       }else if(isAndroid){
            $(document).on("tap ",".ios,.android",function(){
              if(cookies){
              ...
              setTimeout(function(){ window.location.href = 'http://a.app.qq.com/o/simple.jsp?pkgname=com.tyrbl.wujiesq';  },800); 
            });
       }
       //写入cookie
