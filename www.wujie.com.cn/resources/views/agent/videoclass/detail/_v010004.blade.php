...
@section('main')
  <section class="containerBox bgcolor " id="containerBox">
    ...
    <div class="bgwhite fix_bottom">
        <input value=" 写评论..." class="comment tl color666" type="button" style="background: transparent;">
      ...
      Zepto(function(){
        ...
        function getVideodetail(id,uid){
             var param = {};
                 param.id = id;
                 param.uid = uid;
             var url=labUser.agent_path +'/lessons/detail/_v010004';
             ajaxRequest(param,url,function(data){
                if(data.status){
                  ...
                  if(data.message){
                    ...
                    var vodHtml ='';
                    getVod(detail.url,0);
                    $('.videotitle').text(detail.subject);  
        ...
        //获取热门评论
          function getComment(id,uid,page,pageSize){
             var param = {};
                 param.id = id;
                 param.uid = uid;
                 param.type ='Lesson';
             ...
             var url=labUser.agent_path +'/comment/comment-list/_v010004';
             ajaxRequest(param,url,function(data){
                if(data.status){
                  var comHtml='';
                  if(data.message.data.length >0){
                    $.each(data.message.data,function(i,j){
                      comHtml +='<li class="fline tl mb1-5"><div class="l mr1"><img src="'+j.avatar+'" class="com_header"></div><div class="l width85"><p class="lh3-3 color666 f13 ">'+j.nickname+'</p><p class="f15 ">'+j.content+'</p><div class="color999 mb1"><span class="l">'+timeForm(j.created_at_time)+'</span><span class="r ui-zan-zone" data-id="'+j.id+'">'
                      if(j.is_zhan == '0'){
          ...
          getComment(id,agent_id,page,pageSize);
          /**实例化点播**/
          ...
          $(document).on('click','.loadcomment',function(){
            ...
            if(statu == "1"){
              page++;
              getComment(id,uid,page,pageSize)
            }
