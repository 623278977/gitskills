...
@section('endjs')
    ...
    <script type="text/javascript">
      Zepto(function(){
        ...
        // 跟进品牌
          function followBrand(agent_id,customer_id){
              var url = labUser.agent_path +'/customer/detail-brands/_v010002';
        ...
        //意向加盟        
          function getIntentDetail(){
            var url = labUser.agent_path + '/customer/customer-intention-record/_v010002';
            ajaxRequest({'agent_id':agent_id,'customer_id':customer_id},url,function(data){
              if(data.status){
        ...
        //跟单备注
          function getDocumentary(agent_id,customer_id){
            var url = labUser.agent_path +'/customer/detail-remarks/_v010002';
            ajaxRequest({'agent_id':agent_id,'customer_id':customer_id},url,function(data){
                if(data.status){
      ...
      //活动邀请
          function getEvents(agent_id,customer_id,type){
            var url = labUser.agent_path +'/customer/activity-invite/_v010002';
    ...
    //考察邀请
          function getInspects(agent_id,customer_id){
            var url = labUser.agent_path +'/customer/inspect-invite/_v010002';
            ajaxRequest({'agent_id':agent_id,'customer_id':customer_id},url,function(data){
                if(data.status){
                        ...
                        $.each(list,function(index,item){
                            if(item.inspect_list){
                              $.each(item.inspect_list,function(i,j){
                                ...
                                }else if(j.status == 1 || j.status == 2 || j.status == 3){
    ...
    //合同加盟
          function getPactdetail(agent_id,customer_id){
              var url = labUser.agent_path + '/customer/contracts/_v010002';
              ajaxRequest({'agent_id':agent_id,'customer_id':customer_id},url,function(data){
                if(data.status){
                    ...
                    if(list.length >0){
                      $.each(list,function(i,j){                    
                            if(j.status == -1 || j.status == -2){
                              ...
                              pactHtml += '<div class="flex_bet pr1-5 color666 f12 align-c pt1"><p>缴纳状态:<span class="cffa300">待确认 </span>('+j.leftover+')</p><p><button class="pact_sendagain" data-id="'+j.id+'" data-title="'+j.contract_title+'" data-brand="'+j.brand+'" data-amount="'+j.amount+'" data-type="'+j.type+'">再次发送</button></p></div></div><div class="bgf5 pt1-5"></div></div>';
