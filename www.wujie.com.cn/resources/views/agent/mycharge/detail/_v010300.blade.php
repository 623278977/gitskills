...
@section('main')
    <section class="containerBox none bgcolor" id="containerBox" >
        ...
        <div>
          <button class="withdraw"  onclick="withdraw()" >提现</button>
        </div>
    </section>
@stop
@section('endjs')
    ...
    <script type='text/javascript'>
       ...
       Zepto(function(){
          ...
          var agent_id = args['agent_id'] || 0,
            ...
            function myCharge(agent_id,page,page_size,months){
                var url = labUser.agent_path + '/user/my-commission/_v010300';
                var param = {};
                  param['agent_id'] = agent_id;
                  param['page'] = page;
                  param['page_size'] =page_size;
                  param['month_chioces'] = months;
                ajaxRequest(param,url,function(data){
                  if(data.status){
                      ...
                      var qua = obj.month_chioces[0];
                        $('.selected').text(qua);
                      var quaHtml = '';
                      if(obj.month_chioces.length >0){
                          $.each(obj.month_chioces,function(i,j){
                            quaHtml += '<li>'+j+'</li>';
                          })
                      };                   
          ...
          //加载更多
          function getMore(agent_id,page,page_size,months){
              var url = labUser.agent_path + '/user/my-commission/_v010300';
              var param = {};
                  param['agent_id'] = agent_id;
                  param['page'] = page;
       ...
       function recordIntro(obj,qua){
          ...
          var config = {
           ...
           $(document).on('click','.countdetail li',function(){
              ...
              window.location.href = labUser.path + 'webapp/agent/mycharge/datas/_v010300?id='+id+'&type='+type+'&log_id='+log_id;
           });
