...
@section('endjs')
    ...
    Zepto(function () {
        ...
        $(document).on('click','.brand_join',function () {
            if (uid==0) {
                ...
                window.location.href=labUser.path+'webapp/brand/join?&id='+id+'&uid='+uid;
            }
        })
        function shareDetail(result) {
            ...
            if (shareFlag) {
                    ...
                    if (is_weixin()) {
                        ...
                        var desptStr = removeHTMLTag(selfObj.summary);
                    ...
                    $(document).on('click','.brand-pinpai .brand-contain',function () {
                        ...
                        window.location.href= labUser.path+'webapp/brand/detail?id='+brandid+'&is_share=1&pagetag=02-1-2';
                    });
                    //查看品牌资讯详情
                    $(document).on('click','#brand_zx .brand-news',function () {
                        var zxid = $(this).data('id');
                       window.location.href= labUser.path+'webapp/headline/detail?id='+zxid+'&is_share=1&pagetag=02-4';
                    })
            }else{
                //查看更多相关品牌
                ...
                $(document).on('click','.brand-pinpai .brand-contain',function () {
                    var brandid = $(this).data('id');
                    window.location.href= labUser.path+'webapp/brand/detail?pagetag=02-1-2&id='+brandid+'&uid='+uid+'';
                });
                //查看品牌资讯详情
                $(document).on('click','#brand_zx .brand-news',function () {
                    var zxid = $(this).data('id');
                   window.location.href= labUser.path+'webapp/headline/detail?pagetag=02-4&id='+zxid+'';
                })
                //打电话
   ...
   <script>	
        ...
        function showShare() {
            ...
            var img = $('#brand_t_img').attr('src').replace(/https:/g,'http:');
            var header = '品牌';
